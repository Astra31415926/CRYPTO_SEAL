...
function generateMandala() {
    ...
    const modSize = 512 / (modCount + 12);
    
    els.ctx.fillStyle = "rgb(200, 200, 200)";
    els.ctx.fillRect(0, 0, 512, 512);
    els.ctx.fillStyle = "rgb(50, 50, 50)";
    els.ctx.fillRect(modSize * 4, modSize * 4, 512 - modSize * 8, 512 - modSize * 8);

    const grid = build7x7Grid(modCount + 4);
    for (let r = 0; r < modCount + 4; r++) {
        for (let c = 0; c < modCount + 4; c++) {
            let valB = 50;
            if (r >= 2 && r < modCount + 2 && c >= 2 && c < modCount + 2) {
                if (qr.isDark(r - 2, c - 2)) valB = 200;
            }
            els.ctx.fillStyle = `rgb(${grid[r][c]?200:50}, ${grid[r][c]?200:50}, ${valB})`;
            els.ctx.fillRect(c * modSize + modSize*2, r * modSize + modSize*2, modSize + 0.5, modSize + 0.5);
        }
    }
...
function build7x7Grid(size) {
    ...
    for (let by = 0; by < Math.ceil(size/14); by++) {
        for (let bx = by; bx < Math.ceil(size/14); bx++) {
            const p = SPRITE_LIB_7x7[Math.floor(Math.random()*10)];
            for (let py=0; py<7; py++) for(let px=0; px<7; px++) if((p[py] >> (6-px)) & 1) octantMap.set(`${bx*7+px-3},${by*7+py-3}`, true);
        }
    }
    ...
    reader.onload = (event) => {
        ...
        img.onload = () => {
            ...
            if (code && code.data.startsWith('|')) {
                ...
                if (raw[1] === els.keyInput.value) {
                    els.textInput.value = raw[0];
                } else {
                    drawSpy();
                }
            } else {
                els.textInput.value = code ? code.data : "QR не найден";
            }
        };
